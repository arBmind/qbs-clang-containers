...
RUN apt-get update -qq \
  && DEBIAN_FRONTEND="noninteractive" apt-get install wget gnupg -yy
...
RUN wget -qO - https://apt.llvm.org/llvm-snapshot.gpg.key | apt-key add - \
  ...
  && apt-get update -qq \
  && DEBIAN_FRONTEND="noninteractive" apt-get install clang-${CLANG_MAJOR} lld-${CLANG_MAJOR} libc++abi-${CLANG_MAJOR}-dev libc++-${CLANG_MAJOR}-dev -yy --no-install-recommends \
...
RUN apt-key adv --keyserver keyserver.ubuntu.com --recv-keys C65D51784EDC19A871DBDBB710C56D0DE9977759 \
  && echo "deb http://ppa.launchpad.net/beineri/opt-qt-${QT_VERSION}-${DISTRO}/ubuntu ${DISTRO} main" > /etc/apt/sources.list.d/qt.list \
  && apt-get update -qq \
  && DEBIAN_FRONTEND="noninteractive" apt-get install git make libgl1-mesa-dev qt${QT_MAJOR}script qt${QT_MAJOR}base qt${QT_MAJOR}tools -yy
...
RUN DEBIAN_FRONTEND="noninteractive" apt-get install ${RUNTIME_APT} -y \
  && qbs setup-toolchains --detect \
  && qbs config defaultProfile clang \
  && qbs config --list
